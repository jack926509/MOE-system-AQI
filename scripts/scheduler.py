...
from system_b_air import freshness as freshness_mod
from scripts import prune_old as prune_mod
...
def make_jobs(settings: Settings, db: Database, notifier: TelegramNotifier | None):
    ...
    admin_chat = settings.telegram.chat_ids.get("admin")
    plant_map = settings.station_to_plant()
    if plant_map:
        logger.info("Plant watchlist: %d 站 covering %d 廠", len(plant_map), len(settings.plants))
    ...
    @_safe("freshness")
    def job_freshness() -> None:
        watchlist = list(plant_map.keys())
        if not watchlist:
            return
        freshness_mod.run_freshness(
            db, notifier, watchlist, admin_chat_id=admin_chat, stale_hours=3,
        )

    @_safe("prune")
    def job_prune() -> None:
        prune_mod.prune(db, days=90)

    return job_aqi_realtime, job_forecast, job_daily_report, job_freshness, job_prune
...
def main() -> None:
    ...
    job_aqi, job_fc, job_daily, job_fresh, job_prune = make_jobs(settings, db, notifier)
    ...
    # job 共用設定：misfire 寬限 10 分鐘、同 job 不重疊、合併補跑
    job_kwargs = dict(misfire_grace_time=600, max_instances=1, coalesce=True)

    # AQI 即時 + 告警：每整點 :15（讓官方完成 publish 後再抓）
    sched.add_job(
        job_aqi, CronTrigger(minute=15, timezone=tz),
        id="aqi_etl_alert", **job_kwargs,
    )
    ...
    sched.add_job(
        ...
        id="daily_report", **job_kwargs,
    )

    # 周界站失聯檢查：每整點 :25（接在 :15 ETL 之後）；無 watchlist 時自動 noop
    sched.add_job(
        job_fresh, CronTrigger(minute=25, timezone=tz),
        id="freshness", **job_kwargs,
    )

    # 保留期清理：每日 03:00 刪 90 天前 raw + VACUUM
    sched.add_job(
        job_prune, CronTrigger(hour=3, minute=0, timezone=tz),
        id="prune", **job_kwargs,
    )
