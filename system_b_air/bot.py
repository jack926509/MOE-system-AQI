...
async def cmd_aqi(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if len(target) > 1:
        ...
        for r in target:
            flag, _ = aqi_flag(r.aqi)
            aqi_str = fmt_num(r.aqi, ".0f")
            lines.append(
                f"{flag} <code>/aqi {html.escape(r.site_name)}</code>　"
                f"{html.escape(r.region)}　AQI {aqi_str}"
            )
...
async def cmd_forecast(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    def _query() -> list[ForecastRecord]:
        with db.session() as session:
            # 先取最新 publish_time，再撈該批次該區的所有預報
            latest_pub = session.execute(
                select(ForecastRecord.publish_time)
            ...
            return list(session.execute(
                ...
                .where(
                    ForecastRecord.region == region,
                    ForecastRecord.publish_time == latest_pub,
                )
            ).scalars())
    ...
    # 同一發布時間內可能有多日預報；對 forecast_date 去重後升冪取最近 3 日
    by_date: dict[str, ForecastRecord] = {}
    for r in rows:
        by_date.setdefault(r.forecast_date, r)
    rows = [by_date[d] for d in sorted(by_date)][:3]
